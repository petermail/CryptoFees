...
export const getUsdAsync = async (onUpdate) => {
    ...
    for (const serverData of serverDataFull) {
        ...
        res = [
            ...
            addData(res, 7, () => createTokenData(FTM, processData(data.data[4]), USDC)),
            addData(res, 8, () => createTokenData(AVAX, processData(data.data[3]), USDC)),
            ...
            addData(res, usdtIndex + 4, () => createTokenData(HT, processData(data.data[24])/1000000000000, USDT)),
            addData(res, usdtIndex + 5, () => createTokenData(MATIC, processData(data.data[7]), USDT)),
            ...
            addData(res, usdtIndex + 8, () => createTokenData(NEAR, processDataNear(dataPost.data[1], 6), USDT)), // ! UP, 
            addData(res, usdtIndex + 9, () => createTokenData(AVAX, processData(data.data[9]), USDT)), // 1 DOWN, 
            addData(res, usdtIndex + 10, () => createTokenData(ALGO, processDataAlgo(dataAlgoUsdt?.data), USDT)),
